...
with open('providers.csv', 'rb') as csvfile:
	...
	for index, row in enumerate(providers):
		if index >0:
			...
			food = Resource.objects.filter(name="food")
			clothing = Resource.objects.filter(name="clothing")
			language = Resource.objects.filter(name="language")
			legal = Resource.objects.filter(name="legal services")
			transportation = Resource.objects.filter(name="transportation")
			medical = Resource.objects.filter(name="medical care")
			school = Resource.objects.filter(name="education and enrollment")
			counseling = Resource.objects.filter(name="counseling")
			housing = Resource.objects.filter(name="housing")
